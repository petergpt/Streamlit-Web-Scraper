...
def get_summary(text):
    text = text.replace("'", '"')
    prompt = f"Please provide a brief summary of the following text. Format it in bullet points. Add emojis as appropriate. At the beginning you need to assess the key sentiment of the text:\n\n{text}"
    messages = [{"role": "system", "content": "You are a helpful assistant that summarises and analyses web pages."},
                {"role": "user", "content": prompt}]
    headers = {'Content-Type': 'application/json', 'Authorization': f'Bearer {os.environ["OPENAI_API_KEY"]}'}
    data = json.dumps({"model": "gpt-4", "messages": messages, "max_tokens": 150, "n": 1, "stop": None, "temperature": 0.5})
